...
def test_https_consumer_successful_poll(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep, sample_run):
...
def test_https_consumer_no_pending_runs(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep):
...
def test_https_consumer_processing_error(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep, sample_run):
    mock_claim_pending_runs.return_value = [sample_run]
    mock_ack_runs.return_value = 1
...
def test_https_consumer_exponential_backoff(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep):
...
def test_https_consumer_backoff_reset(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep, sample_run):
    call_count = [0]
...
def test_https_consumer_ack_all_claimed_runs(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep):
    run1 = {
        "_id": "run_1",
        "payload": {
            "type": "WEBHOOK",
...
def test_https_consumer_ack_failure_skips_processing(mock_claim_pending_runs, mock_ack_runs, mock_time_sleep, sample_run):
    mock_claim_pending_runs.return_value = [sample_run]
